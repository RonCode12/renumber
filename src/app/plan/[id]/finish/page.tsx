...
export default async function FinishStepPage({
  ...
  const plan = await prisma.workPlan.findUnique({
    ...
    include: {
      facebookCampaigns: {
        orderBy: { order: "asc" },
        include: { adsets: { orderBy: { order: "asc" }, include: { ads: { orderBy: { order: "asc" } } } } },
      },
      tiktokCampaigns: {
        orderBy: { order: "asc" },
        include: { adsets: { orderBy: { order: "asc" }, include: { ads: { orderBy: { order: "asc" } } } } },
  ...
  return (
    <WizardShell planId={id} current="finish">
      ...
      <div className="space-y-6">
        <SummarySection title="פרטים כלליים" editHref={`/plan/${id}/general`}>
        ...
        <SummarySection title="פייסבוק / מטא" editHref={`/plan/${id}/facebook`}>
        ...
        <SummarySection title="גוגל" editHref={`/plan/${id}/google`}>
          {plan.googleNotes ? (
        ...
        <SummarySection title="טיקטוק" editHref={`/plan/${id}/tiktok`}>
        ...
        <SummarySection title="פלאשי: SMS ודיוור" editHref={`/plan/${id}/flashy`}>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <ul className="space-y-1 text-xs text-slate-500">
                  {plan.smsItems.map((s: SmsItemRow, i: number) => (
                    <li key={s.id}>
              ...
              <div>
                ...
                <ul className="space-y-1 text-xs text-slate-500">
                  {plan.mailingItems.map((m: MailingItemRow, i: number) => (
                    <li key={m.id}>
